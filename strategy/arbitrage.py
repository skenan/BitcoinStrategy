...
def calculate_price_trend(last_price, current_price):
    # 需要根据最近1分钟的价格变化进行计算
    # False指价格下跌趋势, True指价格上升趋势

    return current_price - last_price > 0
...
def go():
    ...
    while True:
        ...
        huobi_account.cny_balance + okcoin_account.cny_balance, huobi_account.btc_balance + okcoin_account.btc_balance))
        ...
        if (huobi_depth.buy_price - okcoin_depth.sell_price) > 0:  # 获利信号：OKcoin买，huobi卖
            ...
            if Qty:
                ...
                if not calculate_price_trend(last_huobi_depth.buy_price, huobi_depth.buy_price):
                    # 先huobi卖， 再okcoin买
            ...
            if Qty:
                ...
                if calculate_price_trend(last_okcoin_depth.buy_price, okcoin_depth.buy_price):
                    # 先huobi买，再okcoin卖
